...
화가난다. long 범위를 int 로 계속 선언하고 풀어서 오답이였다..
답만 long 으로 바꿔주는게 아니고 답이 될 수 있는 변수인 time 도 long 으로 바꿔줘야했다.
로직은 크게 어렵지 않다. 그냥 완전탐색을 하면된다.
하지만 텔레포트가 양방향인점에 빠져서 간단하게 구현하지 못했다.
일단 순서를 정하고 탐색하며 도착인지 확인하는 로직이 생각한 로직중에서는 최선이였다.

- 모든 텔레포트를 고려해서 순서를 정하고 이동할때마다 도착지까지의 거리를 구하고 최소값을 갱신 <- better
- 텔레포트의 부분집합을 구하고 거기서 순서를 정해 고른 텔레포트를 모두 지난 후 도착지까지의 거리 구하기
